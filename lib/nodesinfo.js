...
var getInfo = function get(nodeURL){
  let requestData = { "jsonrpc" : "2.0", "method": "get_dynamic_global_properties", "params": [settings.witness_account, []], "id": 1 }
  let options = {
        url: nodeURL.url,
        method: "POST",
        json : requestData
        ...
        console.log("No error = "+ noError);
          if(noError){
            for(let i = 0, len = data.length; i < len; i++){
                fullMessage +="<b>"+ data[i].name +":</b>  "+ data[i].result.head_block_number + "\n";
